...
/**
 * Error messages for dynamic glidepath validation failures.
 * These provide clear, actionable feedback to developers and users.
 * @readonly
 */
export const GLIDEPATH_ERROR_MESSAGES = {
  /**
  ...
  AGES: {
    ...
    AGE_DIFFERENCE_TOO_SMALL:
      'Age difference (endAge - startAge) must be at least 1 year',
  ...
  RETURNS: {
    RETURN_TOO_LOW:
      'Return rates must be greater than -0.99 (cannot lose more than 99%)',
  ...
  ALLOCATIONS: {
    WEIGHT_BELOW_ZERO:
      'Allocation weights must be between 0.0 and 1.0 inclusive',
    WEIGHT_ABOVE_ONE:
      'Allocation weights must be between 0.0 and 1.0 inclusive',
  },
  ...
  MODES: {
    ...
    MISSING_REQUIRED_FIELDS:
      'Required fields are missing for the specified glidepath mode',
  },
  ...
  WAYPOINTS: {
    ...
    INVALID_WAYPOINT_VALUE:
      'Waypoint values must be valid for the specified value type',
    TOO_MANY_WAYPOINTS: 'Too many waypoints may impact performance',
    MISSING_EQUITY_RETURN:
      'equityReturn is required when valueType is "equityWeight"',
    MISSING_BOND_RETURN:
      'bondReturn is required when valueType is "equityWeight"',
  },
  ...
  GENERAL: {
    ...
    CONFIGURATION_MISMATCH:
      'Glidepath configuration does not match the specified mode',
  },
} as const;
